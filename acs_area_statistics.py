...
def calc_mask(mask=None, regions=None, ds=None, overlap_threshold=None):
    ...
    if isinstance(mask, xr.DataArray):
        ...
        assert 0.0 <= overlap_threshold <= 1.0, "You have selected min_overlap mask. Please specify overlap_threshold between [0.,1.]"
        if regions is None:
            regions = get_regions(["ncra_regions", "australia"])
...
def calc_stats(
    ...
    for stat in how:
        ...
        if stat.replace("p", "").isnumeric() and (int(stat.replace("p", "")) <= 100):
            ...
            summary_list.append(ds_weighted.quantile(q, dim=dims, keep_attrs=True).drop_vars("quantile").rename(f"{var}_{stat}"))
        elif stat == "mean":
            ...
            summary_list.append(ds_masked.groupby("region").min(dim=dims, keep_attrs=True).rename(f"{var}_{stat}"))
            if bins is not None:
                df_masked = ds_masked.to_dataframe()
                df_masked["category"] = pd.cut(df_masked[var], bins, labels=bin_labels, ordered=True)
                summary_list.append(df_masked.groupby("region")["category"].min().to_xarray().rename(f"{var}_cat_{stat}"))
        elif stat == "median":
            summary_list.append(ds_weighted.quantile(0.5, dim=dims, keep_attrs=True).drop_vars("quantile").rename(f"{var}_{stat}"))
        elif stat == "max":
            summary_list.append(ds_masked.groupby("region").max(dim=dims, keep_attrs=True).rename(f"{var}_{stat}"))
            if bins is not None:
                df_masked = ds_masked.to_dataframe()
                df_masked["category"] = pd.cut(df_masked[var], bins, labels=bin_labels, ordered=True)
                summary_list.append(df_masked.groupby("region")["category"].max().to_xarray().rename(f"{var}_cat_{stat}"))

        elif stat == "mode":
            if bins is not None:
                df_masked = ds_masked.to_dataframe()
                df_masked["category"] = pd.cut(df_masked[var], bins, labels=bin_labels, ordered=True)
                summary_list.append(df_masked.groupby("region")["category"].agg(pd.Series.mode).to_xarray().rename(f"{var}_cat_{stat}"))
            else:
                # mode cannot use the fractional masking in the same way as other statistics
                summary_list.append(ds_masked.to_dataframe().groupby(["region"])[var].agg(pd.Series.mode).to_xarray().rename(f"{var}_{stat}"))
            ...
            if bins is not None:
                ...
                d = {
                    "coords": {
                        "region": {
                            "dims": "region",
                            "data": list(proportion_dict.keys()),
                        }
                    },
                    "dims": "region",
                ...
                summary_list.append(xr.DataArray.from_dict(d).rename(f"{var}_cat_{stat}"))
            else:
                df_masked = ds_masked.to_dataframe()
                proportion_dict = {}
                ...
                summary_list.append(xr.DataArray.from_dict(d).rename(f"{var}_{stat}"))
...
def acs_regional_stats(
    ...
    if chunks is None:
        ds_weighted = ds[var].weighted(mask_x_lat_weights)
        ds_summary = calc_stats(
            ds_weighted=ds_weighted,
            ds_masked=ds_masked,
            var=var,
            dims=dims,
            how=how,
