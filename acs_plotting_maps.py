...
cmap_mustard = LinearSegmentedColormap.from_list("mustard", ["#5a4c10", "#977f1b", "#d3b125", "#e2c85e", "#eddd9b"])
        ...
        if name not in self._regions_dict:
            if name in self.shapefiles:
                ...
                not_region = gpd.GeoSeries(
                    data=[box(*box(*base_region.total_bounds).buffer(20).bounds).difference(base_region["geometry"].values[0])],
...
def crop_cmap_center(cmap, ticks, mid, extend=None):
    ...
    bounds = np.linspace(0.5 * (1 - prop_below_mid), 0.5 * (1 + prop_above_mid), N + 1)
...
def plot_select_area(
    ...
    if select_area is None:
        ...
        not_area = gpd.GeoSeries(
            data=[box(*box(*map_total_bounds).buffer(10 * buffer).bounds).difference(area.dissolve()["geometry"].values[0])],
            crs=ccrs.PlateCarree(),
        )
...
def plot_acs_hazard(
    ...
    ax, norm, cont, middle_ticks = plot_data(
        regions=regions,
        data=data,
    ...
    ax111 = fig.add_axes([0.0, 0.0, 1, 1], xticks=[], yticks=[], in_layout=True)  # (left, bottom, width, height)
...
def plot_acs_hazard_multi(
    ...
    for i, ds in enumerate(ds_list):
        ...
        subtitle = subplot_titles[i]
        ax, _norm, _cont, _middle_ticks = plot_data(
            regions=regions,
            data=ds,
            station_df=station_df,
            markersize=markersize,
